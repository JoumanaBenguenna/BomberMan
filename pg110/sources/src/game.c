...
game_new(void) {
	...
	game->maps[0] = map_decrypte(0);
       	game->maps[1] = map_decrypte(1);
	game->levels = 2;	
       	game->level = 0;
...
void game_banner_display(struct game* game) {
	...
	window_display_image(sprite_get_number(player_get_nb_portee(game_get_player(game))), x, y);
	...
	window_display_image(sprite_get_lvl(), x, y);

	x = 5 * white_bloc + 9 * SIZE_BLOC;
	window_display_image(sprite_get_number(player_get_lvl(game_get_player(game))), x, y);
	...
	while (SDL_PollEvent(&event)) {
		...
		case SDL_KEYDOWN:
			...
			case SDLK_SPACE:
				...
				init_time_bomb(time ,player); 
				break;
			case SDLK_n:
			  if (player_get_lvl(player) > game->level){
			    game->level++;
